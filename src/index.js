...
async function main() {
  try {
    ...
    logger.log(`{green-fg}Found ${wallets.length} accounts to process{/green-fg}`);
    ...
    for (const wallet of wallets) {
      try {
        ...
        logger.log(`{red-fg}Error initializing account: ${error.message}{/red-fg}`);
      }

      dashboard.setAccounts(accounts);
      logger.log(`{green-fg}Waiting 10 seconds before initializing next account{/green-fg}`);
  ...
  setInterval(async () => {
    ...
    for (const [walletAddress, naor] of naoris) {
      try {
        ...
        logger.log(`{red-fg}Error refreshing account ${walletAddress}: ${error.message}{/red-fg}`);
      }
    }
