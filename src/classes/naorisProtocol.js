...
class naorisProtocol {
  ...
  async sendPingRequest() {
    for (const deviceHash of this.deviceHashes) {
      logger.log(
        `{blue-fg}Sending ping for ${this.account.walletAddress} (Device: ${deviceHash}){/blue-fg}`
      );
      ...
      while (retries > 0) {
        try {
          ...
          logger.log(
            `{yellow-fg}Error sending ping for device ${deviceHash}: ${error.message}{/yellow-fg}`
          );
        }
